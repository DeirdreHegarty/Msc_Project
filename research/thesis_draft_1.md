...
The output image generated by Tensorflow contains bounding boxes with labels that display the detected class and percentage of AP (average precision) of each detected object. Extra functionality is added to the Tensorflow sourcecode to facilitate the retrieval of bounding box coordinates, and detected object names as strings. 
...
After the input image has been processed by the Tensorflow backend a python script is triggered. The script retrieves the locations of the detected objects' associated sounds from a JSON array. 
